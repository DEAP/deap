...
def selRoulette(individuals, k):
    ...
    .. warning::
       The roulette selection by definition cannot be used when the fitness
       can be less than or equal to 0.
    """
    s_inds = sorted(individuals, key=attrgetter("fitness"), reverse=True)
    sumfit = float(sum(ind.fitness.values[0] for ind in s_inds))

    if(s_inds[0].fitness.weights[0]>0):
        probs = map(lambda i: i.fitness.values[0]/sumfit, s_inds)
    else:
        # Maps the inverse probabilities and normalizes
        probs = map(lambda i: (1-(i.fitness.values[0]/sumfit))/(len(s_inds)-1),
                    s_inds)
