...
class Ephemeral(Terminal):
    ...
    """
    def __init__(self, generator_id, value, ret):
        self.generator_id = generator_id
        Terminal.__init__(self, value, False, ret)
        
class EphemeralGenerator(object):
    """Class that generates `Ephemeral` to be added to an expression."""
    __slots__ = ('id', 'ret', 'function')    
    def __init__(self, function, ret):
        self.id = hash(self)
...
def mutEphemeral(individual, mode, pset):
    ...
    *mode* ``"one"``, it will change the value of one of the individual
    ...
    if len(ephemerals_idx) > 0:
        ...
        for i in ephemerals_idx:
            ephemeral = individual[i]
            individual[i] = pset.ephemerals[ephemeral.generator_id]()
