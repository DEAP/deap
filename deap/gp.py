...
def SampleChild(pset, minArity_, maxArity_, type_):
    ...
    if maxArity_ > 0:
        candidates += [ p for p in pset.primitives[type_] if minArity_ <= p.arity <= maxArity_ ]
    ...
    arity = node.arity if isinstance(node.arity, int) else 0

    if arity == 0 and isclass(node):
        node = node()
