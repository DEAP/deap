...
class MovingPeaks:
    ...
    *bfunc* can also be given to act as static landscape (the default is no
    ...
    .. plot:: code/benchmarks/movingsc1.py
       :width: 67 %
    """
    def __init__(self, dim, random_state=numpy.random, **kargs):
        ...
        try:
            if len(pfunc) == npeaks:
                ...
                self.peaks_function = self.random_state.sample(pfunc, npeaks)
        ...
        self.peaks_position = [[self.random_state.uniform(self.min_coord, self.max_coord) for _ in range(dim)] for _ in range(npeaks)]

        if uniform_height != 0:
            self.peaks_height = [uniform_height for _ in range(npeaks)]
        else:
            self.peaks_height = [self.random_state.uniform(self.min_height, self.max_height) for _ in range(npeaks)]


        if uniform_width != 0:
            self.peaks_width = [uniform_width for _ in range(npeaks)]
        else:
            self.peaks_width = [self.random_state.uniform(self.min_width, self.max_width) for _ in range(npeaks)]

        self.last_change_vector = [[self.random_state.random() - 0.5 for _ in range(dim)] for _ in range(npeaks)]
    ...
    def changePeaks(self):
        ...
        if self.minpeaks is not None and self.maxpeaks is not None:
            ...
            if u < 0.5:
                ...
                for i in range(n):
                    idx = self.random_state.randrange(len(self.peaks_function))
                ...
                for i in range(n):
                    self.peaks_function.append(self.random_state.choice(self.pfunc_pool))
                    self.peaks_position.append([self.random_state.uniform(self.min_coord, self.max_coord) for _ in range(self.dim)])
                    self.peaks_height.append(self.random_state.uniform(self.min_height, self.max_height))
                    self.peaks_width.append(self.random_state.uniform(self.min_width, self.max_width))
                    self.last_change_vector.append([self.random_state.random() - 0.5 for _ in range(self.dim)])

        for i in range(len(self.peaks_function)):
            # Change peak position
            shift = [self.random_state.random() - 0.5 for _ in range(len(self.peaks_position[i]))]
            ...
            # Change peak height
            change = self.random_state.gauss(0, 1) * self.height_severity
            ...
            # Change peak width
            change = self.random_state.gauss(0, 1) * self.width_severity
