...
def main(procid, pipein, pipeout, sync, seed=None):
    ...
    island = toolbox.population(n=MU)
    ...
    logbook.record(gen=0, island=procid, evals=len(island), **record)
    hof.update(island)
    ...
    if procid == 0:
        ...
        sync.wait()                 # Wait for procid == 0 to log header
        print(logbook.stream)

    for gen in range(1, NGEN):
        island = toolbox.select(island, len(island))
        # Vary the island individuals
        island = list(islice(algorithms.and_variation(island, toolbox, CXPB, MUTPB), len(island)))

        # Evaluate the island individuals
        invalid_ind = [ind for ind in island if not ind.fitness.valid]
        for ind in invalid_ind:
            ind.fitness.values = toolbox.evaluate(ind)

        # Log asynchroneously the islands
        # We have one hall of fame per island
        hof.update(island)
        record = stats.compile(island)
        logbook.record(gen=gen, island=procid, evals=len(island), **record)
        print(logbook.stream)

        # Perform migration after MIG_RATE generations
        if gen % MIG_RATE == 0 and gen > 0:
            toolbox.migrate(island)
